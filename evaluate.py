...
def _map(recs, gt, topn):
    n, ap = 0.0, 0.0
    for u, seen in six.iteritems(gt):
        seen = list(set(seen)) # 정답 봤던거 목록 전체
        rec = recs.get(u, []) # 내 추천 리스트 100개
        if not rec or len(seen) == 0:
            continue
...
def _entropy_diversity(recs, topn):
    ...
    sum_list= []
    for v in six.itervalues(freq):
        #print('freq', v , v / sz * math.log(v / sz) )
        sum_list.append(v / sz * math.log(v / sz))
